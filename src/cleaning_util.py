...
def clean_studentlife_unlock(dfs):
    ...
    for f in list(dfs.keys()):
        ...
        # Fill hours
        df = fill_hours(df).reset_index(drop=True)
        df.loc[pd.isnull(df.start_datetime), 'start_datetime'] = \
            df.loc[pd.isnull(df.start_datetime), 'datetime']
        df['epoch'] = df['datetime'].apply(map_epoch)
        ...
        df['unlock_duration'] = (df['end_datetime_2'] - df['datetime']).dt.seconds    
